...
//! diagnostics (plus convention-resolution checks) onto the LSP protocol.
//!
...
use ulb_lang::eval::{Definitions, collect_definitions};
...
/// `conventions.ulb` (an open document wins over the loader) and report an
/// error for each `apply "name"` whose convention is not defined.
#[derive(Debug)]
...
impl<L: SourceLoader> DiagnosticEngine<L> {
    ...
    fn convention_names(&self, uri: &Url) -> BTreeMap<String, ()> {
        let Some(conventions_url) = uri.join("conventions.ulb").ok() else {
            return BTreeMap::new();
        };
        let Some(text) = self.text_of(&conventions_url) else {
            return BTreeMap::new();
        };
        let parsed = parse(&text);
        let mut defs = Definitions::default();
        let mut diagnostics = Vec::new();
        collect_definitions(&parsed.file, &mut defs, &mut diagnostics);
        defs.conventions
            .keys()
            .map(|name| (name.clone(), ()))
            .collect()
